...
LAG-TOLERANT on another.

**Arm labels are STUDY-LOCAL and are NOT defined here.** Each is defined
in its own study's file under [`pre-registrations/`](pre-registrations/)
or module. The same letter means four different things: `ARM P` is
*persistence* in `emission_timing`, *P&L outcomes* in `macro_event_study`,
the `be_after` *production baseline* in `bear_giveback`, and *portfolio
contribution* in `bear_rewrap`.

**Look one up in [`arm-index.md`](arm-index.md)** — organised by study, so
everything a study owns sits under its own heading; a repo grep mostly
returns one study citing another's arm. Always resolve an arm against the
study that printed it, and name that study when quoting one
(`emission_timing ARM P`, never a bare `ARM P`). A label that reads like
an arm may instead be a sub-arm, cell, control, descriptive cut,
population scope or run; the index tags each one, and the table of
object types is at [arm-index.md#object-types](arm-index.md#object-types).
