...
The operator queued this study on 2026-08-28 ("the hedge programme's next
question is MAX DRAWDOWN, not timing"). The same day's note limits it to their
actual practice, which is exposure-conditional: *"I hedge when I hold a lot of
correlated positions (semis → SMH, tech → QQQ), I see a specific risk, AND the
analysis says people are hedging."*
...
- **Not a timing study.** `hedge_timing` tested market-state triggers (chop /
  ...
  gap-up day, that is incidental; the drafted §4 GAP prohibition is about the
  gap AS the reason to hedge and is not reopened here.
...
- **Not `concurrency_correlation`.** That study (registered 2026-08-22) asks
  ...
  a concentration effect found here does not license a ceiling there, nor the
  reverse.
...
- **Book, RATIFIED (operator, 2026-08-31; consolidated into this file
  ...
  signal dates** — is a REPORTED STRATUM only, never the concluding population.

  An earlier wording of this clause named both readings as if they were one
  population; they are not. The plan-time disclosures below this list — the
  exposure table, the concentration quantiles and the 504-session universe —
  were computed on the `real` stratum alone and describe it, not the ratified
  996-row book. The figures that describe the ratified population are the ones
  the study's own run prints.
...
- **Four clusters are UNHEDGEABLE.** They keep their identity in the
  ...
  **They are NOT folded into BROAD/SPY.** Folding would inflate BROAD's
  ...
  corrupting the trigger itself, and it would contradict
...
- **The sector map is shared, not study-local.** `concurrency_correlation`
  ...
  about what "same sector" means, the failure mode `mapping.CONFIDENCES` and
  `ladder_tier()` exist to prevent. The map above is written to
  `scripts/backtest_study/lib/sectors.py` as the single encoding; whichever
  study is built second imports it rather than restating it.

## Plan-time observations, disclosed

All of the following were measured from INPUTS before any outcome column was
touched. The full working is in
`research/archive/18-hedge-programme-exit-basis-and-text-loop.md` under 2026-08-29.
...
session, over 504 sessions.

| concentration | median | p75 | p90 |
|---|---|---|---|
| any-cluster | 0.301 | 0.398 | 0.572 |
| constituent-only | 0.118 | 0.174 | 0.256 |
...
from. Disclosing this now means a later underpowered result reads as predicted.
...
exactly when needed is not a hedge"* — such sessions are **carried at f=0 and
...
on a prose-conditioned arm alone**.
...
- **ARM P — prose-free counterpart.** ARM C restricted to exactly the sessions
  ARM CS would hedge on, minus the prose condition, to isolate how much of any
  ARM CS effect is the prose rather than the concentration underneath it. As
  registered it does not do that: restricting to CS's sessions and then
  dropping the prose condition still yields CS's sessions, so both arms hedge
  on the same sessions and the arm is **INERT AS REGISTERED**. Redefining it
  into something informative would be a post-hoc arm this registration does
  not cover, so it stays as written.
...
- **ARM R — always-fillable reference.** ARM C with a delta-equivalent SHORT in
  the proxy UNDERLYING instead of a put. It has no option cache dependency and
  no fill gate, so the study cannot terminate on fill coverage alone, which is
  how `hedge_structure` ended. **ARM R is a floor on feasibility, not a
...
- **MEASUREMENT-ONLY** — ARM M shows the two curves differ materially, but no
  hedge cell clears the bar. That is a real, reportable outcome: it would mean
  the programme's prior nulls were measured on a blind instrument, and that no
  hedge mechanism has been found yet.
...
`research/` — draft-and-hold, as the operator chose for the GAP prohibition.
A NULL or CONTRARY verdict ships nothing and is recorded in
