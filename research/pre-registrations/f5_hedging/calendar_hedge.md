...
Everything the study stands on — the book, the code, the eligible dates, the
sizing, the exit and the comparison baselines — is fixed here, before any
number is read.

**Book and code.** `book.load_book(include_bs=False)`; deployed book =
...
used, because you cannot decide to hedge Monday and be filled Friday. The lag
distribution under the loose rule prints as a sensitivity. Excluded and
counted: `entry_net ≤ 0` (crossed/stale market — vol_sleeve saw 2/183) and
`far_exp ≤ near_exp`.
...
`DEBIT_PROD` (pt .90 / sl .75 / tef .75) — the profile the candidate was
measured under. There is no calendar-specific exit; that would stack a second
free parameter on the pick rule.

The frozen harness already handles calendars: the grid ends at the SHORT leg's
expiry, `_price_asof` never carries a leg past its own expiration, and
multi-expiry net marks are deliberately unclamped.

As a LABELLED SENSITIVITY only: the same table under hold-to-near-expiry
(pt/sl/tef all None). It may not change the verdict; it exists so the write-up
can say whether the verdict is exit-shape-dependent.

### Baselines

Two of them — a change from vol_sleeve, which compared vs no hedge only:

- the deployed ladder alone at $50k;
- the ladder PLUS the SHIPPED bear hedge sleeve (`|delta|` descending, ½ size,
  1/day).

The calendar must beat the hedge the operator already has, not just the empty
seat.

## Arms

The pick rules are decision-time only, and the list is CLOSED here.

- **P1** — nearest-ATM: min |K*−S|/S among the day's fillable calendars.
- **P2** — longest near-leg DTE.
- **P3** — shortest near-leg DTE.
- **P4** — widest expiry gap.
- **P5** — same ticker as the day's top-ranked deployed position (P1 tie-break).
- **P6** — ETF underlyings only, then P1.

**THE RULE IS P1** — geometry not score, the closest analogue of the shipped
bear-hedge convention, and implicitly what produced the vol_sleeve number.
P2–P6 print as a robustness fan; a P2–P6 pass with P1 failing is a candidate
for a future window, never a ship.

### ARM S — the structure sweep
...
manifest CSV) into the same cache under the same naming. Synthesis results are
checkpointed to `backtests/sweep_cache/synth_results.csv` so an interrupted run
resumes instead of restarting.

MULTIPLICITY: a sweep cell is a CANDIDATE only if its worst-decile CI excludes
zero at Bonferroni α = 0.05 / (n_structures × n_pick_rules), is right-signed
every year present, and clears H0. **Nothing in ARM S can ship from this run**;
the maximum verdict is carry-to-next-window.
...
These must pass before any hedge number is read.

- **H0 FILL:** P1 produces a fillable hedge on ≥60% of deployed-book dates AND
  ≥60% of the deployed book's worst-decile dates. Both print side by side, and
  the gate fails on either. Unfillable days are carried as f=0 in every
  portfolio line, never dropped from the denominator. A hedge unavailable
  exactly when needed is not a hedge.
- **H0b FRESHNESS:** the headline must survive `stale_at_cap ≤ 3` and
  `pct_real ≥ 0.5` (`vol_sleeve.mark_quality`).
- **RECONSTRUCTION.** Four checks that the plumbing is the plumbing it claims:
  - **R1** — book calibration quoted.
...
Nothing changes `config/backtest.yml`. The maximum outcome is an optional
second hedge sleeve added to `docs/deployment-rules.md` §4, requiring H0 MET ∧
H0b not flipping the verdict ∧ H2 MET ∧ H3 deployable at f ≥ 0.25. Anything
less is a candidate.
