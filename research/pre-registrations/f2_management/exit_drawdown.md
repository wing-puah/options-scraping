...
Train objective: exactly as fixed under "No-lookahead rules, binding" above.
PROD is a grid point here, so tie step (i) is live for ARM W and the full
four-step order applies — this is the one arm for which it does. ARM W reports
BOTH "WF-selected vs PROD" and "PROD itself",
...
- Modelled as **TWO synthetic `Pos` per rec** (half size each, each with its own
  ...
  releases half the reserve at the FIRST exit. This sentence stands as
  registered; the ledger the module actually runs does not implement it, and
  the departure — conservative in direction — is recorded in the errata beside
  this file.
...
- **The dollars ban is scoped to the per-row comparison.** Two plan clauses
  conflict: "Traps to encode" says "Quote R, not dollars, for ARM P (contracts
  change)", while "Unit and metric" makes the dollar MTM max drawdown a
  co-primary reported "for every arm and cell". This registration reads the trap
  as binding ARM P's per-row and paired numbers only, where the contract counts
  differ. The account-level drawdown is a whole-book figure of one \$25,000
  ledger and is reported in dollars for ARM P as for every other arm.
- **This one resolution needs an explicit operator ACK before the module is
  built — an ack, not merely the absence of an objection to a documented
  interpretation.** Elsewhere this file elaborates an unambiguous plan clause;
  here it chooses between two that contradict, and the workflow's phase-1
  instruction ("approving this plan IS the operator's review of the
  commitments; the agent transcribes, it does not redesign") reserves that
  choice for the operator. Under the alternative reading ARM P reports no
  dollar drawdown figure and quotes co-primary 1 in R and as a percentage of
  starting capital. Whichever reading is taken is recorded in this section and
  in the errata beside this file, never left to report prose.
- **STATUS: the ACK is OPEN, and the SCOPED reading above is therefore NOT the
  one in force.** No ACK is recorded, so the module runs the ALTERNATIVE
  reading as its default — ARM P's account-level drawdown, its improvement and
  the improvement's CI bounds print as a share of starting capital, dollars
  only behind `--arm-p-dollars`. Clause 1 is evaluated on the improvement
  RATIO, which is scale-free, so the verdict is identical either way. The
  errata beside this file carries the detail.
...
ledger's marked equity as of the session's open. ARM D is the one arm whose
per-block choices cannot be dispatched per block — `Cfg.dd_throttle` is ONE
value for a whole simulation — so they must collapse to a single value; that
collapse is an EXCEPTION to the per-block dispatch rule above and is recorded
in the errata beside this file.
...
**arm × variant × era × deployment population**. ARM U's two variants (added to
`sl .75`; replacing `sl`) and ARM O's two (OI drop; volume climax) are separate
cells; ARM W, ARM P and ARM D have one variant each. Walk-forward-SELECTED
parameters (`pt`/`sl`/`tef`, `k`, `X`, `d`) are NOT cell axes — they are chosen
per block inside a cell, which is why a cell's report names the per-block
selections rather than a single value. ARM D is the exception noted in its own
section: its `d` cannot be dispatched per block and collapses to one value for
the whole simulation, so its report names both the per-block choices and the
collapsed value.

**Co-primaries**, both reported for every arm and cell (with ARM P's
co-primary 1 subject to its own OPEN item — see ARM P's STATUS above):
...
- **G-CAL — the host simulation is unchanged.** `account_sim`'s own gates
  **G2–G5** must still pass with the DEFAULT replayer
  ...
  not used. The invocation actually run, and the requirement that the per-gate
  lines be PRINTED in this study's own report, are recorded in the errata
  beside this file.
...
- **G1 — LEAK GUARD.** Shift every auxiliary series (bars, OI, volume) ONE
  ...
  - **"ONE SESSION FORWARD" means one session of the RULE'S OWN GRID, not one
    row of the cached file.** An option-history or OHLC file carries dates the
    position's grid never reads — before the signal date, after the exit, and
    any session the WEEKDAY grid skips — so shifting on the file's key order
    pulls a value the rule never saw onto a grid session, which is not "one
    session later" and fires the gate on rows that leak nothing. The shift is
    applied on `t.grid`: session `i` carries what session `i-1` carried,
    MISSING stays MISSING, and keys off the grid are untouched. ARM U
    additionally holds bars at or before the ENTRY session fixed, so the
    entry-frozen ATR14 — a SCALAR computed off bars `<= entry` by construction
    — is not re-estimated; the gate then measures the information set rather
    than the ATR.
  - **Both halves are SCOPED, and the scoping is recorded in the errata beside
    this file**: the DIRECTION half is read on ARM O's volume LEG rather than on
    its conjunction, and the CHANGE half is tallied PER EXERCISED VARIANT rather
    than once per run. G1 is a (V0) gate — read it with the errata, not alone.
...
3. **Stability**: the improvement is **same-signed in both halves** of the window
   **and in ≥ 2 of the 3 years** present.
   ...
   - **No thinness floor is committed here.** This registration sets **NO minimum
     ...
     that can flip a cell straight to `NULL` must be registered in advance, and
     this registration transcribes the Design section rather than adding one. What the
...
4. **Both pricing tiers same-signed** (`real` and `strike_expiry_tweak`).
   - **A tier with NO SIGN cannot be same-signed, and the clause is NOT
     cleared.** A tier with no positions in one of the two books has no
     improvement to compute; it cannot agree with the other, and the clause
     FAILS — exactly as for clause 3 and for the same reason. Clause 4, like
     clause 3, is a STABILITY clause: it asks the PRIMARY evaluated population
     to agree with ITSELF across a cut of its own rows, and a cut that cannot
     agree fails it. Clause 5's vacuous pass is the CORROBORATION case and stays
     the asymmetry argued for there. The reading is PRINTED on the clause-4 line
     of every cell, so a grader reads it rather than infers it.
5. **SECONDARY v3 is not opposite-signed.**
   ...
   - **Vacuous is DISCLOSED, never silent.** When clause 5 is cleared this way
     ...
     affected dates)` with the v3 census beside it. The sidecar the two eras
     exchange, and the further cases the run treats as vacuous, are recorded in
     the errata beside this file — read this clause with it. Any
   ...
   - **The referent is the SECONDARY era's PRIMARY cell, never its `all` cut.**
     "Population and basis" fixes two independent axes — the ERA (v4 PRIMARY, v3
     SECONDARY, never pooled) and the deployment POPULATION (`dense_episodes`
     PRIMARY, `all` a disclosed secondary cut from which **no verdict is read**)
     — and this clause is written on the first axis only. Because `all` carries
     no verdict, an `all` cell is not a verdict-carrying cell and cannot
     contradict one. That is a strict reading of text already here rather than
     new content, and it is stated because it is VERDICT-AFFECTING.
...
- **No deviation is silent.** A build-time deviation from this document is
  folded into the section it amends, so the file keeps stating ONE final design
  rather than a change log; what changed, when and why lives in git. The run's
  own report additionally PRINTS every deviation where it bites, so a reader of
  the report sees it without reading this file. A deviation that would change
  what a gate REFUSES, what an arm DOES, or how a clause is READ is not folded
  on the builder's authority: it is recorded in
  [`../../exit_drawdown-errata.md`](../../exit_drawdown-errata.md), which
  `scripts/study_review/` inlines beside this file as AUTHORITY for the graders,
  and it never relaxes a commitment.
