...
_Status: historical (covers 2026-08-14 → 2026-08-15). Superseded / qualified by: [archive/18 §2026-09-02 `exit_basis` re-measured](18-hedge-programme-exit-basis-and-text-loop.md) — the "`exit_basis` is UNUSABLE" standing hazard is now ERA-SCOPED: correct for the v3-and-earlier exports it was measured on, but v4 is clean (485/485 labelled); [archive/17 §2026-08-22 (late) and §2026-08-27](17-v4-refresh-bear-deploy-and-vocabulary.md) — `selection_order`'s "every arm moves only 7–14% of the book" was a hardcoded prose literal (measured 15–24% PRIMARY / 11–21% SECONDARY), and the thread it left unadjudicable CLOSED on v4 as ORDERING-IS-NOISE. Era scoping, the structure-name fix and the `--live-select` findings stand. Live record: [current.md](../current.md)._
...
(POWER-STOPPED at G0 — nothing read, nothing refuted; its plain-language digest
and both replication reviews are folded in verbatim). The study-suite triage
found the DEBIT_PROD exact-replay gate permanently unsatisfiable and
`bear_position_study`'s R partly contaminated, then FIXED both (the gate now
classifies rows instead of asserting; R is re-replayed). `run --all` went
GREEN with two dead studies retired and "designed refusal" made a runner
status. The `bear put debit spread` structure-name defect was found and fixed
(v4 re-run, v3 left frozen and quantified). `account_sim` gained the
`--live-select` arm. The 2026-08-15 re-export incident — the bare export name
turned out not to name a population — produced `lib/era.py`, and the
`enrich_queue_pilot` cleared its kill switch (9/10 deployed yield), so queues
a/b were declared GO.

The opening block below is the stack of **state-of-play** paragraphs that sat
at the top of `current.md` while these were the newest entries (newest first,
each ending "Prior state follows"). The dated sections that follow are in the
order the log carried them (not strictly chronological — 08-15 fixes were
appended above the 08-14 entries they closed). Paths and figures are as
written at the time; the standing summary is in `current.md`.

---

## State of play as recorded, 2026-08-14 → 2026-08-15
...
[`archive/14-volume-signal-demotion-and-audit.md`](14-volume-signal-demotion-and-audit.md);
70 positions, **$9,852**).
...
Prior state (2026-08-13 and older) is archived — see [`archive/`]() files 07–14 and the [README](../README.md) section index.
...
[`archive/02-credit-debit-split-attempts-8-12.md`](02-credit-debit-split-attempts-8-12.md))
