# Archive 18 — 2026-08-28 → 2026-09-02: the hedge programme, `exit_basis` re-measured, the text ↔ backtest loop

_Status: historical (covers 2026-08-28 → 2026-09-02). Conclusions stand as of 2026-09-04. Superseded / qualified by: [archive/19 §2026-09-04](19-2026-column-exit-drawdown-and-duplicate-repairs.md) and [next-steps.md §2.1](../next-steps.md) — `hedge_concentration` is no longer an ungraded first run: it was re-run on a larger export (ADMITTED 225 / 112 dates) and GRADED under the two-analyst protocol on 2026-09-04 with the PRECONDITION-NULL verdict unchanged, and the max-drawdown hedge question is CLOSED; that entry also records the closing note this volume lacks — everything powered in the hedge programme is about the TRIGGER, and the INSTRUMENT has never been powered, so "not shown to work" is the absence of a measurement, not a measurement of absence; [archive/19 §2026-09-04](19-2026-column-exit-drawdown-and-duplicate-repairs.md) and [next-steps.md §2.0](../next-steps.md) — every "module unwritten" / "no module" statement about `concurrency_correlation` here is superseded: it was built and first-run 2026-09-04, `VERDICT: NOISE`, all 11 powered arms inside ARM N's band; [archive/19 §2026-09-06](19-2026-column-exit-drawdown-and-duplicate-repairs.md) — the §4 gap-up prohibition drafted and HELD here was ACCEPTED by the operator on 2026-09-06, the hedge stays and its trigger is the open item [next-steps.md §2.10](../next-steps.md); [archive/19 §2026-09-07 (fifth)](19-2026-column-exit-drawdown-and-duplicate-repairs.md) and [archive/20 §2026-09-08](20-hedge-programme-reorg-queues-cde-and-cache-loss.md) — `hedge_exposure` is `hedge_portfolio` since 2026-09-08, `hedge_concentration` was merged into it as the `--admitted` arm on 2026-09-07 and its module, pre-registration and record deleted (git holds the files at `44bbfb2`; the DELETED row in [`study-map.md`](../study-map.md#hedging) is the record). The `hedge_timing`, `hedge_exposure`, `exit_basis` and text-loop conclusions stand. Live record: [current.md](../current.md)._

Covers 2026-08-28 through 2026-09-02 — the hedge programme run end to end, and
the two threads that closed beside it. `hedge_timing` tested the operator's own
hedge triggers and killed three of them (GAP-UP CONTRARY, chop and broad decline
NULL, the strict 4–5-day streak UNTESTABLE); a feasibility pass then scoped the
queued max-drawdown study from disk; `hedge_exposure` was built, run and left
deliberately verdict-less, took two errata and an independent F8–F16 audit, and
was RATIFIED on population `all` as UNDERPOWERED + MEASUREMENT-ONLY — the
close-bucketed curve understates this book's max drawdown by 40.2%, which was
then written back against `bear_deploy` D3 / H3 / H4. `study_review` learned to
inline a study's ERRATA file as authority. `hedge_concentration` was registered
off a plan-time census and built and run the same night: PRECONDITION-NULL, and
a POWERED one. On 2026-09-02 `exit_basis` was re-measured — the ban was right
for v3 and WRONG for v4, the proxy half never wrote at all — and then made
AUDITED rather than trusted by three one-directional checks. The volume closes
with the text ↔ backtest loop: text is the last untested column family and it
NULLS, and the model's own stop comes back CONTRARY on bull calls.
...
[`pre-registrations/f5_hedging/hedge_concentration.md`](../study-map.md#hedging),
...
[`study-results/f5_hedging/hedge_concentration.md`](../study-map.md#hedging).
