...
_Status: historical (covers 2026-07-22). Superseded / qualified by: [archive/09 §"v3 CLOSE-OUT" item 4](09-v3-closeout.md) — the v4 prompt trim DROPPED `score_flow` and `score_dealer`, so "queue item A NOT TRIGGERED, both columns KEPT" decided the fate of columns v4 no longer emits; [next-steps.md §2.2](../next-steps.md) — `v4_bridge` prints `VERDICT: LADDER UNVALIDATED ON v4`, so the ladder's "monotone again at 913, third consecutive validation" is a v3-era result; [archive/18 §2026-09-02 `exit_basis` re-measured](18-hedge-programme-exit-basis-and-text-loop.md) — the `exit_basis` column shipped in addendum 8 reaches every v3-and-earlier export unlabelled and scrambled, and is readable only on v4. The shipped BEAR+H/E trail (addendum 7) and `mech_cell` (addendum 9) still stand on the operator card. Live record: [current.md](../current.md)._
...
- `docs/deployment-rules.md` — new §"Exit management" with the manual live
...
Documented in `docs/backtest-reference.md` §"Exit basis", including the
...
`docs/deployment-rules.md` §"Exit management" states the matching live rule as
