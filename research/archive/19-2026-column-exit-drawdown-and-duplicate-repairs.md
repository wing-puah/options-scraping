# Archive 19 — 2026-09-04 → 2026-09-07: the 2026 column, `exit_drawdown`, and the duplicate repairs

_Status: historical (covers 2026-09-04 → 2026-09-07). Conclusions stand as of 2026-09-09. Superseded / qualified by: [archive/20 §2026-09-08 (later) robustness fold LANDED](20-hedge-programme-reorg-queues-cde-and-cache-loss.md) — the six items this volume's 2026-09-07 (later) robustness-review entry left waiting in two worktrees landed on main 2026-09-08; [archive/20 §2026-09-08 (seventh/eighth) far-call fetch](20-hedge-programme-reorg-queues-cde-and-cache-loss.md) — this volume's 2026-09-07 (fourth) `hedge_structure` entry is qualified, not overturned: after a far-call fetch and a cache restore on a grown export, `hedge_structure` still stops at R2 with the same five rows. Live record: [current.md](../current.md)._

Covers 2026-09-04 through 2026-09-07. `hedge_concentration` was graded under
the two-analyst protocol and closed the max-drawdown hedge question at §2.1;
`concurrency_correlation` was built and first-run the same night, verdict
NOISE, all eleven powered arms inside ARM N's band. The book got its first
2026 signal dates (13 of 166); the full suite re-ran, nothing shipped, but the
per-year clause fired for the first time and campaign b closed. `exit_drawdown`
was registered and run as a new f2 study: walk-forward exit hypotheses came
back UNDERPOWERED on the primary population, and the two cells that were
powered read NULL. `overview.md` and `glossary.md` were rewritten for a reader
who has lost the thread, and the long-dated blind spot was scoped debit-only.
The operator's gap-up hedge prohibition was accepted, and four missing rows
were re-priced; a hedge-open indicator became the next open item. A SPY
duplicate and fifteen older duplicate analysis rows were dropped, leaving a
524-row export. Forty pre-registered dates turned out never to have run, and
the 2026 sample was found to miss its own crash. Twelve stale backtest rows
were dropped, and the backtest gained a guard so it can no longer double a
row. Three doubled analysis dates were repaired, and the pipeline now refuses
a date it has already analysed. A twelve-item robustness review was built:
six items landed on the tree, six waited in two worktrees for the campaign to
finish. `hedge_structure` was checked against the new dates and stayed
blocked at R2, with two cached legs gone missing. The volume closes with the
hedge programme's criteria consolidated into one file and two studies
deleted.

## 2026-09-04 — `hedge_concentration` GRADED and §2.1 CLOSED; `concurrency_correlation` BUILT and first-run: NOISE

Two items. The second is the last registered study that had no module: it was
registered 2026-08-22 and only written today.
...
[X4](../arm-index.md#concurrency_correlation) (era stability) is **not evaluable in a single run** — `lib/era.py` binds one
...
[archive/18](18-hedge-programme-exit-basis-and-text-loop.md), with
...
| attempts table copied out of the archive index | the copy is DELETED. The section now points at [`archive/README.md`](README.md) §Section index, names the two attempts still live in production, says why there is no attempt 14, and notes that the same index continues past 13 in date order into `current.md`. The link had been pointing at `README.md`, which has no such section |
...
[archive/02](02-credit-debit-split-attempts-8-12.md) trail.
