...
| The stored book no longer reproduces under current code — OPEN, not diagnosed | Re-pricing April 2025 moved **8 of 57** stored proxy rows. Three are the exit-fill rule; the other five are `09aa02c`'s entry rule plus a cache deeper than the one those rows were priced on. Two are large: TLT 2025-04-01 `profit_target` day 2 +100% → `expired` day 38 **−754%**, and QQQ 2025-04-28 `stop_loss` day 30 −75% → `direction_only` (unpriced). | look at TLT first — a −754% credit row means its entry moved, and that is the shape `_refuse_debit_priced_to_credit` was written for. Settle this BEFORE any deliberate re-price of the book, because a re-price moves more than the exit-fill rule alone. |
