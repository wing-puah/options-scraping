...
| The debit-to-credit gate also fires on BS-modelled legs | `_refuse_debit_priced_to_credit` cannot tell a modelled mark from a quoted one, and in the proxy's method-2 `bs` tier a modelled SHORT leg can legitimately price above a real long leg. Such a play is now refused rather than written. `proxy.bs_fallback` is OFF by default, so no row in the current book is affected — but a deliberate `bs_fallback: true` study will see fewer rows than it did before 2026-09-19. Two committed test fixtures built exactly this shape and had to be corrected. | note it in any `bs_fallback` study's write-up; decide whether the gate should exempt a modelled leg |
