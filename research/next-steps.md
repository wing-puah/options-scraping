...
| The exit fill keeps the sign-independent liquidation mark | The 2026-09-19 entry change stops at the entry. Every daily mark, including the one the exit is taken at, still uses `_zero_bid_mark`, so closing a LONG leg into a 0 bid receives `ask/2` rather than 0. A liquidation mark is defensible for a mark-to-market path — and the exit DAY is chosen by rules that read that path — so this is not obviously wrong, only unexamined. **Census taken 2026-09-19:** across both tabs, 79 of 2,707 exit-day leg quotes are one-sided (2.9%), touching 72 of 1,375 rows (5.2%). Approximate — the exit day is reconstructed from `days_held` over a business-day grid rather than read from the stored grid — so treat it as an order of magnitude, not a row list. | **Operator direction 2026-09-19: use the NEXT DAY if possible.** Rather than fill a one-sided exit at either `ask/2` or the 0 bid, carry to the next day that has a genuine two-sided quote and fill there. Not built. Two things to settle first: what happens when no such day arrives before expiry, and that moving the fill day moves the exit DAY too, which the exit rules read off the same marked path. |
