...
| 2026-08-28 — `hedge_timing` (f4, NEW): GAP-UP CONTRARY, §4 prohibition drafted and HELD; chop / broad decline NULL; strict streak UNTESTABLE | [archive/18](archive/18-hedge-programme-exit-basis-and-text-loop.md) |
| 2026-08-29 — feasibility pass for the queued max-drawdown hedge study (design notes, not a registration) | [archive/18](archive/18-hedge-programme-exit-basis-and-text-loop.md) |
| 2026-08-31 — `hedge_exposure`: built, run, verdict-less by design; two errata; independent audit F8–F16; RATIFIED population `all` → UNDERPOWERED + MEASUREMENT-ONLY (curve understates max DD by 40.2%) | [archive/18](archive/18-hedge-programme-exit-basis-and-text-loop.md) |
| 2026-08-31 — `study_review` inlines a study’s errata as authority; ARM M’s understatement recorded against D3 / H3 / H4 | [archive/18](archive/18-hedge-programme-exit-basis-and-text-loop.md) |
| 2026-08-31 — `hedge_concentration` REGISTERED off a plan-time census, then BUILT and RUN the same night: PRECONDITION-NULL, and a POWERED one | [archive/18](archive/18-hedge-programme-exit-basis-and-text-loop.md) |
| 2026-09-02 — `exit_basis` re-measured: the ban was right for v3 and WRONG for v4 (485/485 labelled); the proxy half never wrote at all | [archive/18](archive/18-hedge-programme-exit-basis-and-text-loop.md) |
| 2026-09-02 — `exit_basis` now AUDITED, not trusted: three one-directional checks, reporting only | [archive/18](archive/18-hedge-programme-exit-basis-and-text-loop.md) |
| 2026-09-02 — the text ↔ backtest loop BUILT and first-run: text NULLS (`exit_from_text` CONTRARY on bull calls, `prompt_eval` at the variance floor); thread closed | [archive/18](archive/18-hedge-programme-exit-basis-and-text-loop.md) |
