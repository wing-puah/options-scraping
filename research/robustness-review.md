...
| B5 | A zero bid falls through to the last trade, usually the more conservative mark. But in a tail of about 5 to 8% of zero-bid rows the leg stays valued when its liquidation value is near zero. | BUILT 2026-09-07 in worktree `.claude/worktrees/wf_129cdac5-757-5`, reviewer-approved, UNMERGED until the queue C/E campaign ends ([log](archive/19-2026-column-exit-drawdown-and-duplicate-repairs.md#2026-09-07-later--robustness-review-twelve-items-built-six-landed-in-the-tree-six-wait-in-two-worktrees-for-the-campaign-to-end)) | Low | Mark a zero-bid long leg at 0 when the ask is also 0; otherwise `ask/2`. | S |
...
| A4 | Output validation checks one key (`regime`). No per-play required keys or enums. Zero plays is logged, not refused. The retry loop logs which attempt won but persists nothing. | BUILT 2026-09-07 in worktree `.claude/worktrees/wf_129cdac5-757-6`, reviewer-approved, UNMERGED until the queue C/E campaign ends ([log](archive/19-2026-column-exit-drawdown-and-duplicate-repairs.md#2026-09-07-later--robustness-review-twelve-items-built-six-landed-in-the-tree-six-wait-in-two-worktrees-for-the-campaign-to-end)) | Med | Validate `plays` and per-play enums; persist attempt count in the manifest. | S |
...
| # | What actually happens | Checked | Size | Fix | Effort |
|---|---|---|---|---|---|
| P1 | Closing a spread inverts its label. The journal classifies on the fill's sign, not the resulting position. Reproduced: closing an NVDA 170/180 bull call spread journals `bear_call_spread`, confidence `NONE`, tier `VETO`. Every closed spread journalled before the fix carries this. | FIXED 2026-09-07 (`6d98608`). Repair APPLIED 2026-09-22 (`aa30549`, `journal repair --merge-drive --apply`): 10 pre-fix CLOSE rows re-derived, 156 rows out, 0 refused ([entry](current.md#2026-09-22-evening--journal-repair-lands-cost-model-on-five-queue-items-decided)) | Med | Classify on the position after the fill, or skip classification on `CLOSE` (`scripts/journal/s02_reconcile.py`, `scripts/journal/lib/mapping.py`). Re-derive past rows. | M |
