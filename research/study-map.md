...
| `hedge_exposure.py` | When the open book is **concentrated** in one correlated cluster, does a long put on that cluster's proxy cut the book's **mark-to-market** drawdown, versus carrying the same concentrated book unhedged? | **UNCONCLUDED — no verdict, on purpose.** The first run (2026-08-29) printed one; a three-lens audit the same day found the defects behind it, written up in [`hedge-exposure-errata.md`](hedge-exposure-errata.md) and now fixed. Two of them are why this cell is empty of a result. **ERRATUM 1:** the registration's population clause names two different books — the literal `load_book(include_bs=False)` call and a row/date count only the raw `BacktestResults` stratum matches — and which one is read decides how many cells are powered at all. The study therefore prints gates, cell shape and cell-level words under **both** populations, every count computed at run time, and emits **no study-level verdict**; the operator ratifies a reading before anything is recorded here or in `study-results/`. **ERRATUM 2:** ARM P is degenerate as worded (ARM C on ARM CS's sessions, minus the prose, *is* ARM CS's sessions), so the only control on the disclosed model-recall lookahead does not exist and the binding prose rule is unreachable by construction — ARM P is declared **inert as registered** and has not been redefined. Also fixed, and not yet read from: CONTRARY now carries the positive bar's clause set sign-inverted; G-MTM reconciles against the row's **stored** `realized_pnl_abs` rather than the replay it took the exit index from; the month-shuffle bootstrap is replaced by a chronological moving block; ARM RF prints as **UNREGISTERED — ADDED AFTER COMMIT**. Nothing ships and nothing is claimed. |
