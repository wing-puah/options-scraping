...
| `staged_exit.py` | Evaluate ONCE at a fixed session X on P&L vs the ORIGINAL entry — exit, tighten the stop, or arm a trail — where the reactive drawdown-from-peak rules of Attempts 1/2/10 failed. | **NULL on both eras.** v3 2026-08-19 (795/118): 60/96 cells UNDERPOWERED, all 36 powered cells fail the CI outright. Re-run on v4 2026-08-27 (sha 25f3e27, 996 rows / 145 dates) on a better-powered book and it says the same thing: `40 of 96 cells clear the floor; 56 are UNDERPOWERED.` and not one powered cell reaches CANDIDATE or REACTIVE-AGAIN — every one is a bare `-`. The closest still fails at criterion 1 (ARM E X=15, `DeltaR +0.008`, `CI95 [-0.005, +0.020]  FAIL`). Continuation shares 49–79% show time-staging bought no immunity from the reactive mechanism. The Attempt-1/2/10 null extends to scheduled switches. |
...
| File | The question | Verdict |
|---|---|---|
| `bear_deploy.py` | Bear selection is unfixable — but is bear worth holding as a **hedge**? Four estimands: D1 joint selection×exit, D2 hedge contribution, D3 sizing, D4 conditional pick. | Bear = a hedge, not a selection — that half is unmoved (D1 NOT MET on every run). But the hedge case REVERSED on the refreshed v4 export: since 2026-08-24 every estimand reads **NOT MET** (D2 flipped MET → NOT MET between the 08-22 and 08-24 runs; D3 never met at any size; D4 the \|delta\|-descending pick reads −0.004 vs the day average, CI [−0.166, +0.166]). The §4 pick line was **PULLED** and the sleeve is held as **operator policy** ([`deployment-rules.md` §4](../docs/deployment-rules.md)), not on v4 evidence. The v3 D2 MET / D4 ADOPTED read is recorded in [`deployment-evidence.md`](deployment-evidence.md). |
...
| `selection_order.py` | `account_sim`'s *rejected* picks outperformed its taken ones. Does a different **blind entry-side order** of the same candidate set spend the scarce delta budget better — or was that read an artifact? | **ORDERING-IS-NOISE** (2026-08-27, v4, 140 dates) — thread CLOSED. All four arms clear G0 on the PRIMARY population (O1 29, O2 26, O3 31, O1b 32 affected dates against the pre-declared floor of 25; each re-ordering moves 15–22% of O0's taken picks), and none clears all seven parts: every arm sits inside the O4 random-order band (p95 +0.057; best arm at the 84th percentile). The adverse-ordering read was an ARTIFACT of which picks the cap happened to exclude. Earlier runs (v3, and v4 through 08-24) were UNDERPOWERED at G0; the "7–14%" figure those quoted was a hardcoded prose literal, corrected 2026-08-22. |
| `hedge_timing.py` | The bear hedge sleeve is deployed on discretionary triggers — chop, a SPY gap-up, a 4–5-day SPY down-run. Does any of them, made mechanical, pick a day the hedge beats the **same day's** ladder-eligible long? | **RUN 2026-08-28** (era v4 sha 1fe4923; v3 replication beside it), graded. **TIMING-CANDIDATE survivors: 0 of 9** (~0.45 expected by chance). GAP-UP is **CONTRARY** on both money arms — `HEADLINE difference -0.408  CI95 [-0.749, -0.057]` (H3, every LOO fold, both years, all cuts) and `best gated policy f=0.50  delta total $-5,893` with max DD and worst date identical to never hedging (H4) — so the drafted §4 prohibition (*do not open the hedge on a gap-up day*) is **HELD for the operator** per the registration. CHOP: NULL/NULL/UNSTABLE. DECLINE-BROAD: NULL on all three arms, which the pre-registered asymmetric rule reads AGAINST the strict habit. The operator's own 4–5-day streak rule stays **UNDERPOWERED** exactly as fixed in advance by the registration (this book samples **2** such dates; no direction is ever quoted from n=2). v3: H3-GAP underpowered by ONE date (24 vs 25), H4-GAP NULL but directionally consistent incl. 2026 (−$2,640). **Nothing ships**; forward trigger: ≥25 strict-streak dates or ≥25 post-2025-11-04 dates. |
...
| `concurrency_correlation.py` | `max_positions_per_day` caps the FLOW of new positions; nothing caps the **STOCK** of open ones. Does the SIZE and internal SIMILARITY of the open book degrade per-position outcome, independently of what was selected? | **NOISE** (first run 2026-09-04, era v4): `all 11 powered arms sit inside ARM N's band. Neither the SIZE of the open book nor its internal similarity degrades per-position outcome on this era's deployed book, at any ceiling on either grid.` PRIMARY 3 dense episodes / 87 dates / 218 positions; SECONDARY 129 dates / 321 positions. `K 3` and `K 5 / same-underlying` are UNDERPOWERED (21 and 7 moved dates against the pre-declared floor of 25) and print census only; **ARM CK NOT RUN** — neither component clears alone. Three things the run established rather than assumed: the book is **long-only** (`{1: 321}`, 321/321 with same-direction count == open count), so `ARM K / same-direction` **IS ARM C** on a different grid and is excluded from the conjunction; **X7's delta control barely discriminates** (110 of 129 dates in `[2.0,inf)`, leaving ONE readable band, and one band is the whole sample re-labelled, not a control); and ARM D0's DESCRIPTIVE shape is **not flat** (`[0,3)` +0.4533 → `[6,10)` −0.1256 by same-direction-and-sector count) but is registered descriptive-only and every ceiling arm that would act on it sits inside the null band. **X4 is not evaluable in one run** (`lib/era.py` binds one run to one era), so every arm is CAPPED at CANDIDATE-PENDING-X4 and the `--era v3` companion command is printed. Nothing ships. |
| `portfolio_delta.py` | Is the book's net delta LEVEL a lever? Dose-response by exposure band, a ceiling-band admission, and a delta-TARGETED hedge sleeve, all against a seeded random-admission null band. | **The read REVERSED on 2026-08-27.** v3 2026-08-19, and the v4 runs of 08-22 and 08-24, all printed **NOISE**. The 08-27 v4 run (sha 25f3e27, 996 rows / 145 dates, 317 deployed picks over 127 dates) prints **CANDIDATE-FOR-INDEPENDENT-WINDOW**: `B ceiling 1.00` (`+0.1106 R  CI95 [+0.0135, +0.2262]`, LOO `share>0 100%  MIN +0.0739`, 69 moved dates, `pct 100%` of the ARM N band) and `B ceiling 1.50` (`+0.0894 R  CI95 [+0.0255, +0.1656]`, `MIN +0.0640`, 43 moved dates) clear the full seven-part conjunction, and B 1.50 clears it on the SECONDARY full book too (`+0.0872 R  CI95 [+0.0283, +0.1526]`). ARM D still reads `shape: not monotone / not readable`. **NOTHING SHIPS** and no ceiling value may be adopted on its P&L — the label queues an INDEPENDENT WINDOW and nothing else. Long-only-by-construction got sharper: `negative-delta picks 0 of 317   per-date net/equity range [+0.00, +2.49]`. |
