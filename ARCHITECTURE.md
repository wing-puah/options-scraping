...
scripts/                    ← entry points, each maps to a workflow step
  ...
  backtest.py               — analysis-driven: reads analysis plays → models each as signed legs
                              ...
                              (config/backtest-reference.md). Shared internals (analysis load,
...
(scratch, gitignored); conclusions in `config/backtest-tuning/current.md`; metric definitions
in `config/backtest-tuning/glossary.md`; the replication protocol in
`config/backtest-tuning/replication-protocol.md`.
...
non-default `--config` run also rebuilds `docs/account-sim-charts.html` from that arm.
...
**`scripts/study_map/`** — renders `docs/study-map.html`: what each study asks (`catalog.py`,
...
- `IVpct` (Barchart options-overview IV percentile, 0–100 — the rich/cheap read that picks
  ...
  the analysis row
- a market-level **Hedge pressure** score (0–100) — see `config/conviction-score.md`
