...
- **MFE / MAE (Max Favorable / Adverse Excursion)** — the best (MFE) or
  ...
  `config/backtest-reference.md`). MFE tunes the profit target, MAE tunes
...
- **Tier ladder A/B/C/VETO** — `book.ladder_tier()`, per
  `config/deployment-rules.md`: **VETO** = never emitted
...
- [`config/backtest-reference.md`](../backtest-reference.md) — raw column
  definitions for `BacktestResults`/`BacktestProxy` (`realized_pnl_pct`,
  `mfe_pct`, `exit_basis`, etc. on the underlying row).
- [`config/rollup-reference.md`](../rollup-reference.md) — per-ticker
...
- [`README.md`](README.md) — how to run a study and where its write-up
  lands.
- [`config/deployment-rules.md`](../deployment-rules.md) — the operator
