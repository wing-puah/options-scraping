...
- **Every verdict on this page was read on the 166-date book of 2026-09-04,
  except `ladder_overlay`, closed 2026-09-16 on its own later run.** The 42
  dates of queues C, D and E landed 2026-09-08 and no other study has run
  since, so the rest of the numbers below are the last recorded ones, not
  this book's. The suite is stale and is to be re-run once, deliberately
...
| [`ladder_overlay`](study-results/f3_structure/ladder_overlay.md) | **NULL**, closed 2026-09-16 | no ladder or naked-put cell beats the plain spread on v4; the six powered v3 cells are NULL too | [plan](pre-registrations/f3_structure/ladder_overlay.md) |
...
graded cells are NULL on v4; on v3 six powered cells are NULL and four are
...
of an R against the plain spread. Waiting for a gap-up or a run before
selling is the only pattern that is positive on v4, but it still fails three
criteria — the CI includes zero, 2026 is a negative year, and it correlates
positively with the deployed book, the re-wrap pattern `financed_spread`
prints above. Closed 2026-09-16; nothing ships, and the thread reopens only
on genuinely new dates
...
- **`ladder_overlay`** ([§2.13](next-steps.md#s2-13)) — **CLOSED 2026-09-16**.
  ...
  v4; on v3 six are NULL and four are UNDERPOWERED. Nothing ships; the
