...
_Status: historical (covers 2026-08-22 → 2026-08-27). Superseded / qualified by: [archive/18 §2026-08-31 `hedge_exposure`](18-hedge-programme-exit-basis-and-text-loop.md) — the `bear_deploy` D3 verdict graded here still stands, but it was read on a close-bucketed curve that UNDERSTATES this book's max drawdown by 40.2%, now recorded as a limitation of its measurement basis in [deployment-evidence.md](../deployment-evidence.md); [archive/19 §2026-09-04](19-2026-column-exit-drawdown-and-duplicate-repairs.md) — the `concurrency_correlation` registered here was BUILT and first-run on 2026-09-04: `VERDICT: NOISE` on both eras, no arm clears X2 or X3, thread closed; [archive/20 §2026-09-08](20-hedge-programme-reorg-queues-cde-and-cache-loss.md) — `bear_deploy` is `hedge_sizing` and `hedge_exposure` is `hedge_portfolio` since 2026-09-08 (labels and figures unchanged). Live record: [current.md](../current.md)._
