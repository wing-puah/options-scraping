  ...
  "plays": [
    {
      ...
      "pattern": "TF|TF-S|MR|GE|VC|PU|DP — the playbook (Step 2). TF (trend following) and GE (gamma expansion) bias WITH the trend/breakout direction; TF-S (trend following, slow grind — the Step 4 rich-IV case) biases WITH the trend too; MR (mean reversion) bias OPPOSITE the extension; PU (positioning unwind) bias WITH the unwind direction; VC (volatility compression) and DP (dealer pinning) are non-directional. HP is NOT a value here — it lives in the market regime. The `structure` direction MUST match this playbook's bias.",
      ...
      "structure": "Option structure from the two-layer table (Step 4): playbook fixes the bias, IV picks aggressive/moderate/conservative. Bullish: long call / call spread / short put. Bearish: long put / put spread / short call. Vol expansion: straddle / strangle / calendar. Vol compression or DP: short strangle / iron condor / butterfly. TF/MR with time-structure edge: diagonal spread or calendar. TF-S (trend following, SLOW grind: high IVpct + positive-gamma/contango, no catalyst) takes a bull put spread if bullish, NOT a debit. NEVER emit a bear call spread (suspended 2026-07-13 — backtest-vetoed as toxic; express a bearish slow-grind read as a bear put spread instead, or pass). Structure is chosen ONLY by playbook + IV — never by conviction level; do not de-risk a high-conviction thesis into a credit structure. NAME THE STRUCTURE FROM THIS EXACT VOCABULARY, then the strikes: bull call spread | bear put spread | bull put spread | bear call spread (vetoed, never emit) | long call | long put | short put | short call | straddle | strangle | short strangle | iron condor | butterfly | calendar | diagonal. NEVER put the word `debit` or `credit` inside the name — write `bear put spread 350/320`, NOT `bear put debit spread 350/320`, and `bull put spread 400/385`, NOT `bull put credit spread 400/385`. The debit/credit nature is implied by the name and belongs in the thesis if it needs saying. A qualifier inside the name breaks the downstream structure parser, which then backtests a two-leg vertical as a single long option. e.g. bull call spread 185/200",
      "thesis": "one sentence",
...
- Playbook–structure binding (validity gate, not a score gate): the `structure`
  ...
  bear call spread is vetoed above — so take a bear put spread (which is a
  debit; the word `debit` still never goes inside the name) or pass.
