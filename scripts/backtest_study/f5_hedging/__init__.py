...
or underpowered.
    ...
    hedge_sizing.py     SHIPPED — bear selection is unfixable, but bear pays on
    ...
    hedge_structure.py  open — the vol sleeve's one survivor, the calendar,
    ...
    hedge_portfolio.py  open · UNDERPOWERED on the mechanism, MEASUREMENT-ONLY on
                        ...
                        `run hedge_portfolio -- --admitted`; a bare run does both
...
surviving questions — with what, when, and how much.
    ...
    vol_sleeve.py       DELETED 2026-09-07 — null · CLOSED. The straddle cleared
                        ...
                        `hedge_structure`, which rebuilds that cell in-process
                        under R4. The verdict is the DELETED row in
                        research/study-map.md; its registration and record were
                        deleted 2026-09-08 (git 44bbfb2). The synthesis layer
                        R4 still runs is lib/sleeve_synth.py.
    hedge_concentration.py
                        DELETED 2026-09-07 — merged into `hedge_portfolio.py` as
