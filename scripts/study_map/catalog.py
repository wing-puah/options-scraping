...
STUDIES: dict[str, Study] = {
    ...
    "exit_drawdown": Study(
        ...
        verdict="RE-RUN 2026-09-05 (era v4, 1,143-row book / 166 dates; SECONDARY era v3 run "
                ...
                "primary population`), which both powered cells read as VACUOUS — "
                "`v3 cell UNDERPOWERED — no sign to contradict with` on `ARM O/vol`, "
                "`v3 cell SECONDARY-UNDERPOWERED — no sign to contradict with` on "
                "`ARM D/throttle`: v3 neither "
                "corroborates nor contradicts v4. GRADED the same day under the two-analyst "
                "protocol: A and B agree on every row and the validator found no violations; "
                "the one gap is that B never graded the arm-level token row, which is "
                "recorded as the report prints it. The finding that outlives the tokens is "
                "that the IN-FAMILY BEST IS NOT STABLE ACROSS BLOCKS — on the `all` cut the "
                "walk-forward re-picks a different winner as the train window grows "
                "(`ARM W/wf` `{'pt 0.90 / sl off / tef off': 3, 'pt 1.10 / sl off / tef off': 2, "
                "'pt 1.10 / sl off / tef 0.75': 2}`, `ARM U` `{'k 3.0': 3, 'k 1.5': 4}`, "
                "`ARM O/oi` `{'X 0.40': 3, 'X 0.25': 4}`, `ARM D` `{'d 0.10': 3, 'd 0.05': 4}`) "
                "— and that the in-sample DISCLOSURE block sizes the tuning bias every earlier "
                "in-sample exit read carried (PRIMARY best $-4,127..$-5,275 vs shipped $-5,466; "
                "`all` $-8,118..$-13,167 vs $-14,238), from which no criterion is evaluated. "
                "The RECORDED run is a further re-run at sha `efd9b76`, after `SHIPPED_BE_AFTER` "
                "was pinned to `config/backtest.yml` (`None` while `structure_exit` is "
                "disabled): identical verdicts and cells, and the baseline basis line now reads "
                "`base -> BEAR_HE (the bear-debit be_after block is DISABLED in "
                "config/backtest.yml, so no breakeven stop is merged)` instead of naming the "
    ...
    "bear_rewrap": Study(
        ...
        verdict="The two naive re-wraps are dead and the DIAGONAL is a candidate that keeps "
                "failing the year. On the 2026-09-19 run (sha 6d8dfc6): `bear debit rows: 483  "
                ...
                "cut is not, since `ex_2026_feb_apr` drops almost nothing (`n= 261` -> `n= 252`), "
    ...
    "financed_spread": Study(
        ...
        question="Does financing a book debit vertical with a credit position pay — an "
                 ...
                 "credit vertical?",
        verdict="Nothing ships, and the RE-WRAP token has MOVED. On the 2026-09-19 run (sha "
                "6d8dfc6; `era v4   book 1325 rows / 207 dates   2024-01-10 .. 2026-05-07`, "
                ...
                "of 919 candidates (`F4-d20 $100      candidates  919  built  114`, "
    ...
    "ladder_overlay": Study(
        ...
        verdict="No. Every graded cell prints `NULL`, unchanged across the 2026-09-10, 09-16 and "
                "2026-09-19 runs (latest sha 6d8dfc6, `kept 447 cores of 447 bull_call_spread "
                ...
                "`ladder_overlay-v3-2026-09-16.txt`) prints `NULL` on its six powered cells and "
                "`UNDERPOWERED` on the four T0 cells; its `N-ROLL` is `PAIRED       n= 124 /  69 "
    ...
    "hedge_structure": Study(
        ...
        verdict="Gates pass; the primary stays unreadable; the FILL RATE is what binds. The "
                "2026-09-19 run (sha 6d8dfc6) is the first since 2026-09-08 — the study was "
