...
INFRA: dict[str, str] = {
    ...
    "lib/capital_adequacy.py": "The OUTCOME-BLIND capital census behind account_sim's "
                   "CAPITAL ADEQUACY block (feasibility plan step 0c): the per-contract "
                   "max-loss distribution of a set of plays, and the share of them whose ONE "
                   "contract fits a given capital's risk budget, per ladder rung and "
                   "inverted to the smallest capital a 50/75/90% share needs — a ceil-index "
                   "ORDER STATISTIC, not the interpolated quantile the census prints, which "
                   "lands one play short of the target; the achieved share is printed beside "
                   "it. It reads max_loss_per_contract and nothing else — no P&L, no exit "
                   "reason, no record of what the walk took. Its population is the LOADED "
                   "book (real + tweak, bs excluded) or a cut of it, NOT every play the "
                   "analysis emitted: load_book has already dropped rows it could not price "
                   "through their exit, which conditions on post-entry data, so the caller "
    ...
    "lib/path_bootstrap.py": "The moving/circular BLOCK BOOTSTRAP of account_sim's A3 max "
                   ...
                   "mtm_curve.max_drawdown rather than reimplementing it, and divides by "
                   "STARTING capital exactly as A3 does, so the band and the criterion "
                   "measure one thing. It resamples the realized per-session P&L: sizing, "
                   "cap refusals and the dollar stop are NOT re-simulated, so a resampled "
                   "path could not have happened under the account's own rules. Separate "
                   "from lib/forward_drawdown.py's block bootstrap, which resamples PAIRED "
                   "rows off a fixed grid to CI a state-vs-outcome statistic. NO DECISION "
                   "RULE: a wide band never licenses a pass, and nothing reads its output.",
