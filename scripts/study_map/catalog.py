...
STUDIES: dict[str, Study] = {
    ...
    "exit_drawdown": Study(
        ...
        verdict="RUN 2026-09-05 (era v4, sha e19d3b4, 1,143-row book / 166 dates; SECONDARY "
                "era v3 also run and recorded the same day, 795-row book / 118 dates) — "
                "UNDERPOWERED across the board, the modal outcome the registration named IN "
                "ADVANCE. Quoted verbatim from the v4 report's own VERDICT SUMMARY: "
                ...
                "'SECONDARY-UNDERPOWERED': 1}`. Every cell failed G0 on the OOS-stitched "
                "population before any drawdown or ΔR clause was evaluated — the walk-forward "
                "split leaves too few TEST dates behind the burn-in to clear the 25-date / "
                "60-row floor at any arm, not just the harder ones: `ARM W/wf` cleared only "
                "`19 rows / 16 dates`, `ARM W/prod` (the PROD grid point itself) changed "
                "`0 rows / 0 dates` at all, and even the best-populated cell, `ARM O/vol`, "
                "reached only `35 rows / 28 dates` against the 60-row floor. The v3 SECONDARY "
                "run has no OOS dates at all — every arm reads `(no OOS dates)` — so it "
                "cannot corroborate or contradict v4 either way. Nothing ships from this "
