...
For the conviction score that ranks tickers, see `docs/conviction-score.md`.
...
> (settlement IV as of the trade date D) by `scripts/collector/fetch_counterpart_iv.py`, stored in a
...
- `docs/conviction-score.md` — scoring rubric (how rollup fields map to the 0–12 conviction score)
- `docs/barchart-reference.md` — raw column definitions from the Barchart CSV
- `config/prompts/analysis-framework.md` — how the LLM uses these columns in its 5-step process
