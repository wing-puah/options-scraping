...
python3 scripts/backfill_iv.py --backfill            # every compiled date (idempotent)
python3 scripts/backfill_iv.py --backfill --dry-run  # report scope, no scrape/upload
python3 scripts/backfill_iv.py --date 2026-06-26 --force      # clear sidecar and re-fetch
...
lib/                        ← shared modules, imported by scripts, never run directly
  ...
  iv_backfill.py            — pure logic for the IV-spread counterpart backfill: which missing legs to fetch (`needed_counterparts`), the per-date sidecar schema/name, and the `build_iv_lookup` the rollup folds in. Shared by scripts/backfill_iv.py (producer) and lib/flow_summary/core.py (consumer) so contract keys + IV units always agree
...
scripts/                    ← entry points, each maps to a workflow step
  ...
  backfill_iv.py            — the paper-faithful IV spread needs a matched call+put at the SAME (strike, expiration); the traded flow almost never carries both legs (→ IVspr ~98% blank on flow alone). For each single-sided in-window (10–60 DTE) (strike, expiry) that traded, scrape the MISSING opposite leg's Barchart price-history (same fetch_history_fast path as enrich_oi) and extract its settlement IV / OI / volume / delta AS OF trade date D. Store one row per fetched counterpart in a per-date Drive sidecar `iv-backfill-{YYYYMMDD}.csv` (schema `lib/iv_backfill.BACKFILL_COLUMNS`; IV in points). Idempotent/resumable (a contract with a non-blank `fetched_on` is skipped, incl. empty ones; --force clears). The pure logic (which counterparts to fetch, the sidecar lookup, the shared contract key) lives in `lib/iv_backfill.py`; the rollup reads the sidecar via `build_iv_lookup` and folds the counterpart legs into `_flow_ticker_rows`' matched-pair + skew accumulators. Date-keyed so backtest (historical D) and live (latest D) share one path. Run daily after enrich_oi.
