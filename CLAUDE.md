...
- **Per-play `regime` and `signal` are ticker-specific, never copies of the market read.** The
  ...
  in sync. Pinned by `test_analysis_pipeline.py::test_analysis_to_rows_never_inherits_market_regime`
  (regime) and `::test_analysis_to_rows_expands_plays_and_drops_blank_ticker` (signal).

- **`scripts/backtest_study/harness.py` is FROZEN — do not edit.** Every recorded conclusion in
  `config/backtest-tuning/current.md` was produced by this exit-replay engine; changing it
  silently invalidates the entire tuning history rather than failing loudly. Enforced by a
  `permissions.deny` rule in `.claude/settings.json`, not just by this paragraph. If a study
  genuinely needs different replay behaviour, add a new engine beside it and say so in the
  report's provenance header.

- **The research tier never enters production.** `scripts/backtest_study/` is never imported by
  `lib/`, `scripts/analysis_pipeline/`, or `scripts/backtest*` and is never scheduled. Its
  reports argue about the book; they do not feed it.

- **Adding a column to `ROW_COLUMNS` means updating the tab headers too.** The
  AnalysisClaude/AnalysisGPT/AnalysisTickerSpecific headers must gain the column or new rows
  write an unlabelled trailing column. Check with `python3 scripts/align_tab_headers.py --dry-run`.
